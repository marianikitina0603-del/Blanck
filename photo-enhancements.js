(function(){
  ...
  function setupStudent(){
    ...
    style.textContent=`
      ...
      .photo-count{font-size:13px;font-weight:700}.photo-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px;margin-top:12px}
      .photo-item{position:relative;border:1px solid #ddd;border-radius:9px;padding:6px;background:#fafafa}.photo-item img{width:100%;height:105px;object-fit:cover;border-radius:6px;display:block}
      ...
      .photo-label{width:100%;margin-top:6px;padding:7px 8px;border:1px solid #ccc;border-radius:7px;font:12px Arial,sans-serif;background:#fff}
    ...
    card.innerHTML=`<h3>Фотографии решения</h3><div class="photo-help">Можно прикрепить до 10 фотографий. Каждую фотографию можно подписать, например «Задание 20» или «Черновик». Поддерживаются JPG, PNG, WEBP, HEIC/HEIF. Размер одной фотографии — не более 10 МБ.</div><div class="photo-picker"><input id="photoInput" type="file" accept="image/*" multiple><span id="photoCount" class="photo-count">0 / 10</span></div><div id="photoGrid" class="photo-grid"></div>`;
    ...
    function render(){
      ...
      selected.forEach((entry,i)=>{
        ...
        item.innerHTML=`<button type="button" class="photo-remove" title="Удалить">×</button><img alt="Фото ${i+1}"><div class="photo-name">${esc(f.name)}</div><input class="photo-label" type="text" maxlength="80" placeholder="Подпись, например: Задание 20" value="${esc(entry.label||'')}">`;
        ...
        const labelInput=item.querySelector('.photo-label');
        labelInput.disabled=sending||submitted;
    ...
    sendBtn.onclick=async()=>{
      ...
      const paths=[];
      const labels=selected.map(x=>(x.label||'').trim());
      sendBtn.disabled=true;input.disabled=true;sendBtn.textContent='Отправка…';render();
      try{
        ...
        sending=false;
        if(!submitted){sendBtn.disabled=false;input.disabled=false;render()}
      }
    };
  }
  ...
  function setupTeacher(){
    ...
    if(typeof originalOpen==='function'){
      window.openWork=async function(id){
        ...
        const q=await client.from('submissions').select('form_state,photo_paths,photo_labels').eq('id',id).single();
        if(!q.error&&blank){
        ...
        const labels=Array.isArray(q.data&&q.data.photo_labels)?q.data.photo_labels:[];
        ...
        signed.data.forEach((x,i)=>{
          const a=document.createElement('a');
          a.href=x.signedUrl;a.target='_blank';a.rel='noopener';
          a.style.cssText='display:block;border:1px solid #ddd;border-radius:9px;padding:6px;background:#fafafa;color:inherit;text-decoration:none';
          const title=(labels[i]&&String(labels[i]).trim())?labels[i]:'Фото '+(i+1);
          a.innerHTML='<img src="'+esc(x.signedUrl)+'" alt="'+esc(title)+'" style="width:100%;height:180px;object-fit:cover;border-radius:6px;display:block"><div style="font-size:13px;font-weight:700;margin-top:7px">'+esc(title)+'</div><div class="muted" style="margin-top:2px">Фото '+(i+1)+'</div>';
          grid.appendChild(a)
        });
